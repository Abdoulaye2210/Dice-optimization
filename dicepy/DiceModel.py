...
#    @njit(cache=True, fastmath=True)
    def simulateDynamics(self, x, sign, outputType, num_times,
        ...
        sigma = 0.01
        tsept = 5
        num_times = round(T/tstep)
        t = np.linspace(1, T, num_times)
        W = np.random.standard_normal(size = num_times) 
        W = np.cumsum(W)*np.sqrt(dt) ### standard brownian motion ###
        X = (mu-0.5*sigma**2)*t + sigma*W 
        #Y = Y0*np.exp(X) ### geometric brownian motion ###
        ...
        for i in range(2, num_times+1):
           ...
           #YGROSS[i] = al[i] * ((L[i]/MILLE)**(1.0 - gama)) * K[i]**gama
            YGROSS[i] = Y0*np.exp(X[i])   ### geometric brownian motion ###
