...
#    print(scc_period, e_bump, c_bump)
    ...
    ##z_ = np.random.standard_normal((num_times+1,1))
    #z_ -= z_.mean()
    #z_ /= z_.std()
    #SS = np.zeros(num_times+1)
    #SS[0] = S0_
    #for i in range(1, num_times+1):
      #SS[i] = SS[i-1] * np.exp((r_ - sigma_ ** 2 / 2) * dt + sigma_*math.sqrt(dt) * z_[i]) ### Goemetric Brownian motion
    #YGROSS[1:num_times+1,] = SS
    #plt.plot(t,S);
    #plt.show
...
################################################################################
    ...
    X = (mu_-0.5*sigma_**2)*t + sigma_*W 
    YGROSS[1:num_times+1,] = YGROSS_0*np.exp(X)[1:num_times+1]  ### geometric brownian motion. We have supposed that YGROSS 
                                                                ### takes YGROSS_0*np.exp(X) values between 2015 to 2515 ###
 ...
 ###########################################################
    ...
    z_ = np.random.standard_normal((M_+1,1))
    z_ -= z_.mean()
    z_ /= z_.std()
    S_ = np.zeros((M_+1,1))
    S_[1] = S0_
    for i in range(2,M_+1):
      S_[i] = S_[i-1] * np.exp((r_ - sigma_ ** 2 / 2) * dt_ + sigma_ * math.sqrt(dt_) * z_[i]) ### Goemetric Brownian motion
    #plt.plot(S_[:, :1]);
    YGROSS[1:num_times+1,] =S_[1:num_times]      ### We suppose YGROSS takes S values between 2015 to 2515
...
############################################################
     ...
     #W = np.cumsum(W)*np.sqrt(tstep)  ### standard brownian motion ###
     #X = (mu_-0.5*sigma_**2)*t + sigma_*W 
     #YGROSS[1:num_times+1,] = YGROSS_0*np.exp(X)[1:num_times+1]  ### geometric brownian motion ###
     #plt.plot(t, Y)
     #plt.show()
############################################################    
