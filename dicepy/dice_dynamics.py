...
###############################################################################
    ...
    for i in range(2, num_times+1):
        ...
        if scc_period == i:
          ...
          CPC[i,] = np.maximum(0.01*np.ones(num_paths), MILLE * np.divide( C[i,] , L[i] )) 
    ...
    if outputType == 0:
         RF = np.zeros(num_paths)
         for i in range(num_times+1):
           RF  = RF + CEMUTOTPER[i,]
        
         resUtility = tstep * scale1 * np.sum(np.sum(CEMUTOTPER)) + scale2*np.ones(num_paths)
         resUtility *= sign
...
#        print(output[0,0])
        ...
        for iTime in range(1, num_times+1):
            ...
            col += 1  # 25
            output26[jTime, ] = MCABATE[iTime,]
            col += 1  # 26
            output27[jTime, ] = C[iTime,]
            col += 1  # 27
            output28[jTime, ] = PERIODU[iTime,]
            col += 1  # 28
            output29[jTime, ] = CEMUTOTPER[iTime,]
            col += 1  # 29
            output30[jTime, ] = MAT[iTime,]
            col += 1  # 30

        return [output1, output2, output3, output4, output5, output6, output7, output8, output9, output10, output11, output12, output13, output14, output15, output16, output17, output18, output19, output20, output21, output22, output23, output24, output25, output26, output27, output28,output29,output30]
