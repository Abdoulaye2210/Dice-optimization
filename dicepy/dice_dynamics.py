...
#    print(scc_period, e_bump, c_bump)
    ...
    ### old_YGROSS is the YGROSS from Dominic's code

    old_YGROSS  = [0, 105.17742198,   125.47307964,   147.66585586,   171.93506037,
   199.30837997,   228.77447473,   260.80160178 ,  296.20605847,
   333.24787519,   375.20974061,   419.01077864,   465.01318811,
   514.80133987,   566.69041402,   620.60180086,   678.61443085,
   739.76978002,   802.8227473,    866.67282008,   935.05406986,
  1009.66027821,  1085.19884012,  1164.05630604,  1244.40540729,
  1320.11706968,  1416.09519332,  1511.69418478,  1606.46453,
  1697.71040571,  1795.81571138,  1897.93744347,  2002.32340153,
  2117.70209657,  2229.89735695,  2346.54021858,  2457.33818118,
  2579.09145283,  2711.39358403,  2839.80782445,  2966.71109162,
  3095.16319175,  3225.45779944,  3371.65358795,  3499.51136467,
  3651.26061384,  3803.7440502,   3956.46548595,  4106.83229726,
  4266.58098265,  4445.52040535,  4625.43367456,  4803.72923149,
  4979.19717338,  5151.31897298,  5319.47977105,  5465.33478475,
  5600.20240114,  5733.64260295,  5872.72743347,  6027.02581232,
  6253.89765962,  6463.96060624,  6576.65535946,  6718.24035457,
  6888.82930896,  7084.79771672,  7300.74532411,  7524.69770082,
  7749.22460104,  7969.67282736,  8183.93494879,  8389.04776007,
  8586.80186817,  8765.44290912,  8921.72932817,  9058.9606512,
  9184.68691565,  9304.09222483,  9418.11297724,  9528.76919747,
  9622.55967018,  9699.07303824,  9741.89025048,  9807.76080169,
  9890.12376309,  9983.62579706, 10083.559547,   10185.73425107,
 10285.40644751, 10378.8736405,  10460.31717187, 10626.11133443,
 10779.11574162, 10922.499924,  11058.51430707, 11188.76114521,
 11314.38338016, 11436.19757527, 11554.78796911, 11670.57327966]


    X = np.zeros(num_times+1)

    S = np.zeros(num_times+1)
    sigma_ = 0.25
    YGROSS_0 = 120
    np.random.seed(1)
    t = np.linspace(2015, 2515, num_times+1)
    W = np.random.normal(0.005, 0.075, 100)
    W = np.cumsum(W)*np.sqrt(tstep) # == standard brownian motion

#W = np.random.uniform(range(0, 99), num_times+1)
# = np.random.uniform(size=101)
#W = np.random.standard_normal(size = num_times+1) 

    X[1] = (np.log(old_YGROSS[1])-0.5*sigma_**2)*0.5 + sigma_*W[1]
    YGROSS[1] = YGROSS_0*np.exp(X[1]) # == geometric brownian motiion
...
######################################################################### GBM
    ...
    for i in range(2, num_times+1):
        ...
        X[i] = (np.log(old_YGROSS[i])-0.5*sigma_**2)*0.5 + sigma_*W[i] 
        YGROSS[i] = YGROSS_0*np.exp(X[i]) # == geometric brownian 
        #####################################################################
